...
http.createServer( function(req, res) {
	...
	} else if (req.url.indexOf('/') > -1 && req.url.replace('/', "") != "") {
		var na = req.url.replace("/", "");
...
}).listen( process.env.OPENSHIFT_NODEJS_PORT || 8080, process.env.OPENSHIFT_NODEJS_IP || "127.0.0.1" ); // Heroku needs port to be set to process.env.PORT, 8080 for local development
...
function pushbox(name) {
	...
	var get_uri = "https://i.l5.ca/" + file_url;
